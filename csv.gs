...
function importCSVFromGoogleDriveThreeTimes(filename, delete_files, load_images, image_names) {
    ...
    fill_column(baseline_sheet, "default error", "=ABS(T2-P2)+ABS(T2-Q2)", 21);
    fill_column(baseline_sheet, "improved error", "=ABS(T2-R2)+ABS(T2-S2)", 22);
    fill_column(baseline_sheet, "error diff", "=U2-V2", 23);
    fill_column(baseline_sheet, "error diff normalized", "=W2/T2", 24);
    fill_column(baseline_sheet, "if error", "=IFERROR(X2, \"\")", 25);
    ...
    range.copyFormatToRange(baseline_sheet, 25, 25, 2, baseline_sheet.getLastRow() - 1);


    if (load_images){
        fill_column(baseline_sheet, "split", "=INDEX(SPLIT(B2, \"-\"),1,1)", 26);

        cell = baseline_sheet.getRange(2, 1, baseline_sheet.getLastRow() - 1, baseline_sheet.getLastColumn());
        cell.sort(26);

        cell = baseline_sheet.getRange(2, 26, baseline_sheet.getLastRow() - 1);
        ...
        for(var i=0 ; i < unique.length ; i++) {
            var rng = baseline_sheet.getRange(2, 26, baseline_sheet.getLastRow() - 1, 2);
            ...
            for (var j=0; j < data.length; j++) {
                if (found_yet == false){
                    if (data[j][0] == search) {
                        found_yet = true;
                        getImage(search, filename, j+1, 28, image_names);
                    }
                }
            }
        }
    ...
    function aggregate(last_row, offset, formula, target){
        var cell = baseline_sheet.getRange(last_row + offset, 25);
        var formulas = [
            [formula]
        ];
        cell.setFormulas(formulas);
    ...
    aggregate(last_row, 2, "=COUNTIF(Y2:Y" + last_row + ",\">0\")", 2);
    aggregate(last_row, 3, "=COUNTIF(Y2:Y" + last_row + ",\"<0\")", 3);
    aggregate(last_row, 4, "=MEDIAN(Y2:Y" + last_row + ")", 7);
    aggregate(last_row, 5, "=AVERAGE(Y2:Y" + last_row + ")", 8);
    aggregate(last_row, 6, "=COUNTIF(Y2:Y" + last_row + ",\"=0\")", 4);
...
function runthis(){
    importCSVFromGoogleDriveThreeTimes("July20-12uhr01", delete_files=false, load_images=true, image_names="log_baseline_validation-July14-19uhr58");
    importCSVFromGoogleDriveThreeTimes("July20-12uhr02", delete_files=false, load_images=true, image_names="lin_baseline_validation-July14-19uhr58");
    importCSVFromGoogleDriveThreeTimes("July20-12uhr03", delete_files=false, load_images=true, image_names="minmax_baseline_validation-July14-19uhr58");
    importCSVFromGoogleDriveThreeTimes("July20-12uhr03", delete_files=false, load_images=true, image_names="best_baseline_validation-July14-19uhr58");
    importCSVFromGoogleDriveThreeTimes("July20-12uhr05", delete_files=false, load_images=true, image_names="log_training_validation-July14-19uhr58");
    importCSVFromGoogleDriveThreeTimes("July20-12uhr06", delete_files=false, load_images=true, image_names="lin_training_validation-July14-19uhr58");
    importCSVFromGoogleDriveThreeTimes("July20-12uhr07", delete_files=false, load_images=true, image_names="minmax_training_validation-July14-19uhr58");
    importCSVFromGoogleDriveThreeTimes("July20-12uhr08", delete_files=false, load_images=true, image_names="best_training_validation-July14-19uhr58");
}
