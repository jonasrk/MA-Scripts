...
def read_and_process_json(file):
    # READ FILE
    with open(file) as f:
        content = f.readlines()
...
all_measurements1 = read_and_process_json(file=sys.argv[1])
all_measurements2 = read_and_process_json(file=sys.argv[2])
...
if generate_plots == "generate_plots":
    ...
    for key in all_measurements2:

        if all_measurements2[key]['coefficient'] != 0.0:
            my_dpi = 96
            ...
            with open('/Users/jonas/Google Drive/suite-logs/July13-23uhr10/est_cards_baseline-July13-23uhr10.csv',
                      'rt') as csvfile:
                spamreader = csv.reader(csvfile, delimiter=';', quotechar='|')
                for row in spamreader:
                    if (row[1].split('-')[0] == key):
                        # print(row[1].split('-')[0])
                        # print(str(int(row[2].replace(',', ''))))
                        # print(str(int(row[5].replace(',', '')) / int(row[2].replace(',', '')) * 1.0))
                        # print(str(int(row[6].replace(',', '')) / int(row[3].replace(',', '')) * 1.0))
                        default_estimator_x.append((int(row[2].replace(',', '')) * int(row[3].replace(',', ''))) ** (1/2))
            ...
            home = expanduser("~")
            plt.savefig(home + '/Google Drive/suite-logs/' + date_id + '/linear_training_validation-' + date_id + '-' + key + '.png', dpi=my_dpi)
            plt.close()
