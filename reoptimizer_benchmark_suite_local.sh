...
if [ $exec_mode = "validation" ]; then
	...
	for filename in 100 20 5 1 0_1
		...
		do
			echo $query
			date
			this_command="tpch.TpcH 'exp(KMeans-$filename-$properties)' $platforms hdfs://thor01/data/csv/TPC-H/tpch_props_$filename.txt $query 0.2 2>&1 | gzip > /Users/jonas/$logfolder/TpcH-$filename-$query-$properties.gz"
	...
	for filename in 500MB.csv 3GB.csv 1GB.csv 125MB.csv 10MB.csv
		...
		date
		echo $input_file
		this_command="kmeans.Kmeans 'exp(KMeans-$filename-$properties)' $platforms $input_file 20 100 2>&1 | gzip > /Users/jonas/$logfolder/KMeans-$filename-$properties.gz"
		eval "$base_command$this_command"
		#sleep 1
	done
	...
	for filename in 0_0625 #0_125 0_25 0_5 2 4 8
		...
		echo $input_file
		date
		this_command="simwords.SimWords 'exp(SimWords-$filename-$properties)' $platforms $input_file 5 5 5 5 2>&1 | gzip > /Users/jonas/$logfolder/SimWords-$filename-$properties.gz"
		eval "$base_command$this_command"
		#sleep 1
	done
	...
	for filename in 0_0625 0_125 0_25 0_5 2 4 8
		...
		echo $input_file
		date
		this_command="wordcount.WordCountScala 'exp(Wordcount-$filename-$properties)' $platforms $input_file 2>&1 | gzip > /Users/jonas/$logfolder/Wordcount-$filename-$properties.gz"
		eval "$base_command$this_command"
		#sleep 1
	done
	...
	for filename in 0_01 0_1 0_5 1 2 5 10 20 50 100
		...
		echo $o_input_file
		date
		this_command="sindy.Sindy 'exp(SINDY-$filename-$properties)' $platforms , '$l_input_file;$o_input_file' 2>&1 | gzip > /Users/jonas/$logfolder/SINDY-$filename-$properties.gz"
		eval "$base_command$this_command"
		#sleep 1
	done
	...
	for filename in page_links_en_uris_de-0_01.ttl page_links_en-0_125.ttl page_links_en-0_25.ttl page_links_en_uris_de.ttl page_links_en_uris_fr.ttl page_links_en_uris_pt.ttl page_links_en_uris_de-0_5.ttl
		...
		echo $o_input_file
		date
		this_command="crocopr.CrocoPR 'exp(SINDY-$filename-$properties)' $platforms $l_input_file $o_input_file 3 2>&1 | gzip > /Users/jonas/$logfolder/CrocoPR-$filename-$properties.gz"
		eval "$base_command$this_command"
		#sleep 1
	done
